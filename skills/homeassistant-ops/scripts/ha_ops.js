    ...
    try {
      ...
      if (jsonBody !== undefined) {
        ...
        if (normalizedMethod === "GET" || normalizedMethod === "HEAD") {
          throw new Error(`${method} ${apiPath} cannot include a request body`);
        }
...
async function updateGroupMembers(rest, configEntryId, members) {
  const optionsFlow = await rest.postJson("/api/config/config_entries/options/flow", {
    handler: configEntryId,
  });
  const flowId = getStr(optionsFlow, "flow_id");
  if (!flowId) {
    throw new Error(`No flow_id in response: ${JSON.stringify(optionsFlow).slice(0, 200)}`);
  }
  ...
  await rest.postJson(`/api/config/config_entries/options/flow/${flowId}`, {
    entities: members,
...
async function cmdRollback(argv) {
  ...
  try {
    ...
    for (const snap of snapshotEntities) {
      ...
      const cur =
        kind === "unique_id" ? currentByUniqueId.get(value) : currentByEntityId.get(value);
      if (!cur) {
        if (kind === "unique_id") {
          warnings.push(
...
function printUpdateGroupsHelp() {
  ...
  console.log("");
  console.log("Apply a rename map to group helper memberships. Entity ID renames");
  console.log("(via the entity registry) do NOT propagate to config-entry-based");
  console.log("group helpers. This command finds affected groups and rewrites");
  console.log("their member lists using the provided rename map.");
  console.log("");
  console.log("Options:");
  console.log("  --map-json <path>    JSON file mapping old entity_ids to new (required)");
  console.log("  --dry-run            Preview changes without applying (default: on)");
...
async function cmdUpdateGroups(argv) {
  ...
  try {
    ...
    for (const entry of groupEntries) {
      ...
      const members = st && Array.isArray(st.attributes?.entity_id) ? st.attributes.entity_id : [];
    ...
    console.log(`${dryRun ? "[dry-run] " : ""}${plan.length} group(s) to update:\n`);
    ...
    const progress = new ProgressReporter(plan.length, "Updating groups: ");
    const failures = [];
