/**
 * Todoist-backed task tool + /todoist command.
 *
...
const TODOIST_TOOL_NAME = "todoist";

const STATUS_KEY = "0-todoist";
...
function setTodoToolActive(pi: ExtensionAPI, active: boolean): void {
  ...
  if (active) {
    if (activeTools.has(TODOIST_TOOL_NAME)) return;
    activeTools.add(TODOIST_TOOL_NAME);
  } else {
    if (!activeTools.delete(TODOIST_TOOL_NAME)) return;
  }
...
async function todoistRequest<T>(
  ...
  try {
    ...
    if (options.signal?.aborted) {
      throw options.signal.reason instanceof Error ? options.signal.reason : new Error("Cancelled");
    }
    if (timeoutSignal.aborted || error?.name === "AbortError") {
...
async function gatherTasks(
  ...
  ctx: ExtensionContext,
  action: ListAction,
  options: { allowPrompt: boolean; operations?: OutboxOperation[]; signal?: AbortSignal } = {
    allowPrompt: true,
  },
  ...
  if (!token) {
    ...
    try {
      workspace = await ensureWorkspace(token, ctx.cwd, options.signal);
      remoteActive = await fetchRemoteActiveTasks(token, workspace, options.signal);
      if (wantCompleted) {
...
async function maybeArchiveWorkspaceSectionIfEmpty(
  ...
  const hasActiveTasks = await hasActiveTasksInWorkspaceSection(token, workspace, signal);
  if (hasActiveTasks) {
...
async function syncOutbox(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  options: { allowPrompt: boolean; notify: boolean; signal?: AbortSignal },
): Promise<SyncReport> {
  if (runtimeState.syncPromise) {
    return waitForPromiseWithSignal(runtimeState.syncPromise, options.signal);
  }
  ...
  let runner: Promise<SyncReport>;
  runner = (async (): Promise<SyncReport> => {
    try {
      const result = await withOutboxLock(ctx.cwd, async (): Promise<SyncReport> => {
          ...
          try {
            ...
            if (isAuthError(error)) {
              ...
              report.warnings.push(
                `Todoist authentication failed while syncing ${operation.type}. Run /todoist setup to refresh credentials.`,
              );
              break;
            }
  ...
  return waitForPromiseWithSignal(runner, options.signal);
}
...
async function runDoctor(ctx: ExtensionCommandContext): Promise<void> {
  const { output, hasToken } = await withSpinnerStatus(
    ctx,
    "running Todoist doctor...",
    async () => {
      const lines: string[] = [];
      ...
      if (!hasToken || !tokenInfo.token) {
        lines.push(
          `- Todoist status: token missing. Set ${TODOIST_TOKEN_ENV} or run /todoist setup.`,
        );
      } else {
        try {
...
export default function todoistExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool(
    defineTool({
      ...
      promptGuidelines: [
        "Use todoist when the user asks to track, inspect, or update tasks instead of keeping an ad-hoc task list in chat.",
      ...
      renderCall(input, theme) {
        ...
        let text = theme.fg("toolTitle", theme.bold("todoist ")) + theme.fg("muted", action);
  ...
  const todoCommandHandler = async (args: string | undefined, ctx: ExtensionCommandContext) => {
    ...
    const ensureNoArgs = (subcommand: string): boolean => {
      ...
      notifyUsageWarning(`/todoist ${subcommand} does not accept arguments.`);
      return false;
    };

    if (!first) {
      notifyUsageWarning("Usage: /todoist <active|pending|completed|all|sync|setup|doctor>");
      return;
    }
    ...
    const message = `Unknown /todoist subcommand: ${first}. Use active, pending, completed, all, sync, setup, or doctor.`;
    notifyUsageWarning(message);
  };

  const commandDescription =
    "Todoist tasks: /todoist <active|pending|completed|all|sync|setup|doctor>";
